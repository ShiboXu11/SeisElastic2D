...
if [ $system == 'slurm' ]; then
    srun -n $ntasks -l -W 0 $SCRIPTS_DIR/Adjoint.sh $velocity_dir $compute_adjoint 2> ./job_info/error_current
elif [ $system == 'pbs' ]; then
...
#mpirun -np $NPROC_SPECFEM --oversubscribe ./bin/sum_kernel.exe $kernel_list,$precond_list $WORKING_DIR $DISK_DIR 2> ./job_info/error_sum_kernel
...
#    mpirun -np $NPROC_SPECFEM --oversubscribe ./bin/xsmooth_sem $sigma_x $sigma_z $kernel_list,$precond_list $DISK_DIR/misfit_kernel/ $DISK_DIR/misfit_kernel/ $GPU_MODE 2> ./job_info/error_smooth_kernel
